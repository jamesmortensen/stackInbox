...
function getAccountIdFromStorage(siteUser, injectScript) {
    ...
    //storage = self.options.stackInboxStorage;
    storage = stackInboxStorage;
    console.debug("getAccountIdFromStorage:: storage = " + JSON.stringify(storage));
    ...
    /*chrome.storage.local.get(null, (function(injectScript) {*/
      //return function(storage) {
        ...
        if(accounts) { 
            ...
            console.debug("getAccountIdFromStorage:: accountId = " + accountId);
            if(!accountId) {
...
//    if(xhr.status == 200) { alert("done")
        ...
        if(!storage.stackInbox) {
            console.info("getAccountId:: storage.stackInbox is empty...");
            storage.stackInbox = {};
 ...
 /*       chrome.storage.local.set(storage, function() {
        ...
        self.postMessage({ type: "FROM_PAGE = " + window.location.hostname, action: "storeAccount", account_id: siteUser.accountId, "storage": storage });        
        self.options.stackInboxStorage = storage;
    }
...
self.on("message", function(addonMessage) {
  // Handle the message
  console.info("content-script:: message received = " + JSON.stringify(addonMessage));
  stackInboxStorage = addonMessage;
  preload(stackInboxStorage);
});
...
console.info("storage = " + stackInboxStorage.newItemCol);//return;
        ...
        if(stackInboxStorage != undefined || stackInboxStorage != null) {
            ...
            console.info("No chrome.storage");
        }
       ...
       /*
        ...
        function updateStorage(newItemColStr) {
            self.postMessage({ type: "FROM_PAGE = " + window.location.hostname, newItemCol: newItemColStr, account_id: stackInboxStorage.account_id });
        ...
        function removeItemFromStorage(removeableItemHref) {
            if (window.localStorage.getItem("newItemCol") != null) {
                currentUnreadItemArr = window.localStorage.getItem("newItemCol").split(",");
                console.info("removeItemFromStorage:: currentUnreadItemArr = " + window.localStorage.getItem("newItemCol"));
